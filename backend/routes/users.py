    ...
    "responses": {
        ...
        "500": {
            ...
            "schema": {
                "type": "object",
                "properties": {
                    "error": {"type": "string", "example": "Database error: connection failed"}
                }
            }
        }
    }
...
def add_user():
    try:
        ...
        organization_name = data.get('organization_id')  # This will now receive organization name
        ...
        if not org_result and organization_name:
            return jsonify({"error": f"Organization '{organization_name}' not found"}), 400
